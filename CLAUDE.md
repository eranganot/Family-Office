...
- The **five**-phase state machine (MAPPING → VERIFICATION → **ALLOCATION** → STRATEGY → MONITORING) is enforced at the API via `workflowGuard`; strategy runs are gated on STRATEGY phase. ⚠️ These notes said "four-phase" and omitted ALLOCATION until 2026-08-02 — M25 inserted it and the docs were never updated. That stale line is the common cause of a family of UI bugs where locked pages named VERIFICATION as the prerequisite for STRATEGY and linked there, sending the owner two phases backwards. **`domain/src/workflow/state-machine.ts` is the only authority; prose is not.** Gates out of VERIFICATION need `verificationComplete` + `suspenseEmpty`; the gate into STRATEGY needs `allocationPlanApproved`.
