import { cleanHebrew, containsHebrew, foldHebrewFinals, repairVisualOrder } from "../text/hebrew";

/**
 * Lives in `domain` because BOTH `ingestion` (which must stamp a merchant key onto
 * every imported transaction) and `engine-operations` (which classifies by it) need
 * it, and the dependency matrix forbids engine -> ingestion. It is pure string logic
 * with no I/O, so this is its natural home regardless.
 */
